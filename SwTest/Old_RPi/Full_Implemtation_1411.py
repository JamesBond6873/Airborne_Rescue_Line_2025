...
def handleButtonPress(button):
    global speedFactor, defaultSpeed, maxSpeedFactor, reverseSpeedFactor, button0count
    ...
    if button == 9:
        ...
        button0count = 1
        print(f"Count = {button0count}")
        
        #printDebug(f"Arm Up")
        #message = f"A1"
        #ser.write(message.encode('utf-8'))
    elif button == 2:
        printDebug(f"Arm Down")
        message = f"A0"
        ser.write(message.encode('utf-8'))
    elif button == 7:
        ...
        if button0count >= 1:
            printDebug(f"SA - Pick Alive Victim -------------------------------------------------------")
            message = f"SA"
            ser.write(message.encode('utf-8'))
            button0count = 0
        else:
        ...
        print(f"--------------------------------Count = {button0count}")
        if button0count >= 1:
            printDebug(f"SD - Pick Dead Victim -------------------------------------------------------------")
            message = f"SD"
            ser.write(message.encode('utf-8'))
            button0count = 0
        else:# Decrease both forward and reverse speed factors
