    ...
    async def send_status():
        while not terminate.value:
            payload = [
                ...
                f"{int(controlLoopFrequency.value)}",
                f"{int(lineCamLoopFrequency.value)}",
                f"{int(serialLoopFrequency.value)}",
                f"{serialCommandPendingConfirmation.value}",
                f"{serialCommandsPendingIndicator.value}",
